...
def update_activation_key(ak_name, row):
    ...
    # Fallback content-view-environments
...
def get_target_product_content_map(ak_name):
    ...
    by_name_product = defaultdict(list)
    ...
    for row in rows:
        ...
        if content_label:
            by_label[content_label] = entry
        if content_name:
            by_name[content_name].append(entry)
        if content_name and product_name:
            by_name_product[(content_name, product_name)].append(entry)

    return by_label, by_name, by_name_product
...
def choose_target_label(exported_label, exported_name, exported_product, by_label, by_name, by_name_product):
    # 1. exact label match
    if exported_label and exported_label in by_label:
        return by_label[exported_label]["content_label"], "matched by content_label"

    # 2. exact name + product match
    if exported_name and exported_product:
        matches = by_name_product.get((exported_name, exported_product), [])
        if len(matches) == 1:
            return matches[0]["content_label"], "matched by content_name + product_name"
        if len(matches) > 1:
            return matches[0]["content_label"], "multiple matches by content_name + product_name, used first"

    # 3. fallback name only
    if exported_name:
        matches = by_name.get(exported_name, [])
        if len(matches) == 1:
            return matches[0]["content_label"], "matched by content_name only"
        if len(matches) > 1:
            # Prefer one with same product if possible, even if product field is blank-ish mismatch
            if exported_product:
                for m in matches:
                    if m["product_name"] == exported_product:
                        return m["content_label"], "matched by content_name only, refined by product scan"
            return matches[0]["content_label"], "multiple matches by content_name only, used first"
...
def main():
    ...
    for ak_name, rows in overrides_by_ak.items():
        ...
        by_label, by_name, by_name_product = get_target_product_content_map(ak_name)

        for row in rows:
            exported_label = pick(row, "content_label")
            exported_name = pick(row, "content_name")
            exported_product = pick(row, "product_name")
            ...
            report.append(
                {
                    ...
                    "message": f"Override {state} applied; {reason}" if result.returncode == 0 else f"Failed to apply override {state}; {reason}",
                    "target_id": ak_id,
                    "content_name": exported_name,
