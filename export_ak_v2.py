...
HC_CSV = os.path.join(OUT_DIR, "activation_key_host_collections.csv")
...
def parse_host_collections_from_info(text: str):
    collections = []
    in_block = False
    ...
    for line in text.splitlines():
        ...
        if raw.startswith("Host Collections:"):
            in_block = True
            continue
...
def main():
    ...
    for ak in aks:
        ...
        content_view_environment = info.get("Content View Environment", "") or info.get("Content View Environments", "")
        ...
        # System purpose / purpose fields
        purpose_role = info.get("Role", "") or info.get("Purpose Role", "")
        purpose_usage = info.get("Usage", "") or info.get("Purpose Usage", "")
        purpose_addons = info.get("Add-ons", "") or info.get("Addons", "") or info.get("Purpose Addons", "")
        ...
        for hc in parse_host_collections_from_info(info_text):
            hc_rows.append(
                {
                    "activation_key_name": ak_name,
                    "host_collection_name": hc,
                }
            )
    ...
    with open(HC_CSV, "w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(
            f,
            fieldnames=[
                "activation_key_name",
    ...
    log(f"activation_key_overrides.csv: {OVERRIDE_CSV}")
    log(f"activation_key_host_collections.csv: {HC_CSV}")
    log("==============================================================")
    return 0
